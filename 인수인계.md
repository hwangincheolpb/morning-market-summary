...
```
아침_시황_요약_자동화/
├── .github/workflows/daily.yml  # GitHub Actions 매일 07:00 KST 실행
├── main.py                      # 메인 실행 스크립트
...
## 🚀 GitHub Actions 배포 (stock_bot과 동일한 방식)

`.github/workflows/daily.yml`이 있어서 **GitHub에 푸시만 하면** 매일 07:00 KST에 자동 실행됩니다.

### 1. GitHub 저장소에 푸시
- 이 폴더를 **새 레포**로 만들거나, 기존 레포에 푸시
- `config.py`는 `.gitignore`에 있으므로 **업로드되지 않음** (올리면 안 됨)

### 2. GitHub Secrets 등록 (필수)
...
| `TELEGRAM_SEND_TO_CHAT_ID` | 받을 내 채팅 ID (숫자만, 따옴표 없이) |

### 3. 동작
- **매일 07:00 KST** (UTC 22:00) 에 자동 실행
- **수동 실행**: 레포 **Actions** 탭 → **Daily Market Summary** → **Run workflow**

### 4. stock_bot과의 차이
- stock_bot: Chrome/ChromeDriver (Selenium) 사용  
- 아침 시황: requests·BeautifulSoup·yfinance만 사용 → **Chrome 설치 단계 없음**
